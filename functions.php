...
//     $nombre_descuento = $config_descuento['nombre_descuento'];
...
//         $fecha_de_reserva = isset($item['booking']['_date']) ? $item['booking']['_date'] : null;
...
//             if ($valorPorcentajeDescuento) {
//                 $descuento_producto = $item['line_total'] * ($valorPorcentajeDescuento / 100);
//                 $descuento_total += $descuento_producto;

//                 // Si el producto es gravable, acumular el descuento gravable
//                 if ($item['line_tax'] > 0) {
//                     $taxable_discounts += $descuento_producto;
//                 }
//             }
//         }
//     }
...
//         $cart->add_fee(__($nombre_descuento, 'woocommerce'), -$descuento_total, false);

//         // Ajustar los impuestos si el descuento afecta productos gravables
//         if ($taxable_discounts > 0) {
//             $taxes = $cart->get_taxes();
//             $subtotal_tax = $cart->get_subtotal_tax();

//             foreach ($taxes as $tax_id => $tax_amount) {
//                 // Recalcular impuestos proporcionalmente al descuento aplicado
//                 $adjusted_tax = $tax_amount - ($taxable_discounts * ($tax_amount / $subtotal_tax));
//                 $cart->set_tax_amount($tax_id, max(0, $adjusted_tax));
//             }
//         }

//         // Ajustar el total enviado a ePayco
//         $cart->set_total($cart->get_subtotal() + $cart->get_subtotal_tax() - $descuento_total);
//     }
// }
// add_action('woocommerce_cart_calculate_fees', 'hz_aplicar_descuento_total');
    ...
    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        ...
        if (mdw_habitacion_tiene_descuento($habitaciones_que_aplica, $product_id)) {
            ...
            if ($valorPorcentajeDescuento) {
                ...
                // // Almacenar el precio original en una variable personalizada
                // $cart_item['data']->set_meta('precio_original', $precio_actual);
...
// function hz_mensaje_descuento_checkout()
// {
//     // Obtener configuraciones del descuento
//     $hotelSettings = get_page_by_path('configuraciones-generales', OBJECT, 'hotel_settings');
//     if (!$hotelSettings) return;
...
//         $mensaje_descuento = sprintf('%s%% de descuento.', $porcentaje_descuento);

//         echo '<tr class="cart-discount custom-discount-row">';
//         echo '<th>' . esc_html($nombre_descuento) . '</th>';
//         echo '<td colspan="2">' . esc_html($mensaje_descuento) . '</td>';
//         echo '</tr>';
//     }
// }

// add_action('woocommerce_review_order_before_order_total', 'hz_mensaje_descuento_checkout');
...
// function hz_mensaje_descuento_checkout()
// {
//     // Texto del mensaje de descuento
//     $mensaje_descuento = __('Ya se ha aplicado un descuento del 30% en los productos seleccionados.', 'woocommerce');

//     echo '<tr class="cart-discount">';
//     echo '<th>' . __('Descuento', 'woocommerce') . '</th>';
//     echo '<td colspan="2">' . esc_html($mensaje_descuento) . '</td>';
//     echo '</tr>';
// }
// add_action('woocommerce_review_order_before_order_total', 'hz_mensaje_descuento_checkout');
...
//                 // Guardar datos del descuento en el artículo del carrito
//                 $cart_item['data']->set_price($nuevo_precio);
//                 $cart_item['original_price'] = $precio_actual; // Precio original
//                 $cart_item['discount_value'] = $precio_actual - $nuevo_precio; // Monto descontado
//                 $cart_item['discount_name'] = $nombre_descuento; // Nombre del descuento
//             }
//         }
//     }
// }
...
// function hz_mostrar_detalle_descuentos_checkout($cart)
// {
//     $total_descuento = 0;
//     $nombre_descuento = '';

//     foreach ($cart->get_cart() as $cart_item) {
//         if (isset($cart_item['original_price']) && isset($cart_item['discount_value'])) {
//             $total_descuento += $cart_item['discount_value'] * $cart_item['quantity'];
//             $nombre_descuento = $cart_item['discount_name']; // Suponiendo que todos usan el mismo descuento
//         }
//     }

//     if ($total_descuento > 0) {
//         // Agregar línea informativa del descuento
//         $cart->add_fee(__('Descuento aplicado: ' . $nombre_descuento, 'woocommerce'), -$total_descuento, false);
//     }
// }
// add_action('woocommerce_cart_calculate_fees', 'hz_mostrar_detalle_descuentos_checkout');

// function hz_mostrar_detalle_descuento_producto($cart_item, $cart_item_key)
// {
//     if (isset($cart_item['original_price']) && isset($cart_item['discount_value'])) {
//         echo '<p style="font-size: 0.9em; color: #666;">';
//         echo 'Precio original: ' . wc_price($cart_item['original_price']) . '<br>';
//         echo 'Descuento aplicado: -' . wc_price($cart_item['discount_value']) . '<br>';
//         echo 'Precio con descuento: ' . wc_price($cart_item['data']->get_price());
//         echo '</p>';
//     }
// }
// add_action('woocommerce_cart_item_subtotal', 'hz_mostrar_detalle_descuento_producto', 10, 2);
...
// function hz_aplicar_descuento_y_mostrar_detalles($cart)
// {
//     if (is_admin() && !defined('DOING_AJAX')) return;
...
//                 $precio_original = $cart_item['data']->get_price();
//                 $descuento_producto = $precio_original * ($valorPorcentajeDescuento / 100);

//                 // Actualizar el precio del producto
//                 $cart_item['data']->set_price($precio_original - $descuento_producto);

//                 // Guardar detalles para mostrar en el checkout
//                 $cart_item['original_price'] = $precio_original;
//                 $cart_item['discount_value'] = $descuento_producto;
//                 $cart_item['discount_name'] = $nombre_descuento;

//                 // Acumular descuento total
//                 $total_descuento += $descuento_producto * $cart_item['quantity'];
//             }
//         }
//     }

//     // Mostrar el descuento total como línea informativa en el checkout
//     if ($total_descuento > 0) {
//         $cart->add_fee(__('Descuento aplicado: ' . $nombre_descuento, 'woocommerce'), -$total_descuento, false);
//     }
// }
// add_action('woocommerce_cart_calculate_fees', 'hz_aplicar_descuento_y_mostrar_detalles');
...
//             if ($valorPorcentajeDescuento) {
//                 // Calcular el descuento sobre el subtotal del producto (sin impuestos)
//                 $subtotal_producto = $cart_item['line_total'];
//                 $descuento_producto = $subtotal_producto * ($valorPorcentajeDescuento / 100);

//                 // Sumar el descuento total
//                 $descuento_total += $descuento_producto;

//                 // Reducir el subtotal del producto en el carrito
//                 $nuevo_subtotal = $subtotal_producto - $descuento_producto;
//                 $cart_item['data']->set_price($nuevo_subtotal / $cart_item['quantity']);
//             }
//         }
//     }

//     if ($descuento_total > 0) {
//         // Agregar el descuento como una línea informativa en el carrito
//         $cart->add_fee(__($nombre_descuento, 'woocommerce'), -$descuento_total, false);
//     }
// }
// add_action('woocommerce_before_calculate_totals', 'hz_aplicar_descuento_total');
