...
?>
<div class="container">
	<div class="hz-calendar-form">
		<fieldset class="wc-bookings-date-picker col hz-calendar <?php echo esc_attr(implode(' ', $class)); ?>">
			<h4 class="hz-subtitle hz-subtitle-before-calendar">Empieza tu experiencia en Amarte</h4>
			<h4 class="hz-subtitle hz-subtitle-calendar">Elige el día</h4>
			<div class="picker hz-calendar-month" data-display="<?php echo esc_attr($display); ?>" data-default-availability="<?php echo $default_availability ? 'true' : 'false'; ?>" data-min_date="<?php echo !empty($min_date_js) ? esc_attr($min_date_js) : 0; ?>" data-max_date="<?php echo esc_attr($max_date_js); ?>" data-default_date="<?php echo esc_attr($default_date); ?>"></div>
			<?php
		...
		<div class="form-field form-field-wide col hz-form-field-calendar">
			<h4 class="hz-subtitle">Selecciona tus opciones</h4>
			<?php
			if ('customer' === $product->get_duration_type()) {
			?>
				<div class="block-picker wc-bookings-time-block-picker">
					<p class="hz-choose-options"><?php esc_html_e('Choose a date above to see available times.', 'woocommerce-bookings'); ?></p>
				</div>
